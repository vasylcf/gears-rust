...
impl ThreadSummaryRepository for MockThreadSummaryRepo {
    async fn get_latest<C: toolkit_db::secure::DBRunner>(
        &self,
        _runner: &C,
    ...
    async fn upsert_with_cas<C: toolkit_db::secure::DBRunner>(
        &self,
        _runner: &C,
        _chat_id: uuid::Uuid,
...
pub fn mock_db_provider(db: Db) -> Arc<DBProvider<toolkit_db::DbError>> {
...
type TestDb = Arc<DBProvider<toolkit_db::DbError>>;
...
pub async fn insert_chat_with_model(
    ...
    secure_insert::<ChatEntity>(am, &toolkit_security::AccessScope::allow_all(), &conn)
        .await
        .expect("insert chat");
}
...
pub async fn insert_test_attachment(db: &TestDb, params: InsertTestAttachmentParams) -> Uuid {
    ...
    let conn = db.conn().unwrap();
    secure_insert::<AttachmentEntity>(am, &toolkit_security::AccessScope::allow_all(), &conn)
        .await
...
pub async fn insert_test_message(db: &TestDb, tenant_id: Uuid, chat_id: Uuid, message_id: Uuid) {
    ...
    let conn = db.conn().unwrap();
    secure_insert::<MessageEntity>(am, &toolkit_security::AccessScope::allow_all(), &conn)
        .await
...
pub async fn insert_test_vector_store_with_provider(
    ...
    let conn = db.conn().unwrap();
    secure_insert::<VectorStoreEntity>(am, &toolkit_security::AccessScope::allow_all(), &conn)
        .await
...
pub async fn insert_test_message_attachment(
    ...
    let conn = db.conn().unwrap();
    secure_insert::<MessageAttachmentEntity>(
        am,

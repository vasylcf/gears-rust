...
2. **The gear's baseline database is PostgreSQL 16 or later; PostgreSQL 19 is a backend capability, not a gear requirement.** Only the SQL/PGQ backend needs 19 — the iterative-CTE and two-query backends run on any supported major. Readiness probes the server major and reports SQL/PGQ as available or unavailable accordingly, the property-graph DDL is applied conditionally on that probe, and an operator who explicitly configures the SQL/PGQ backend on an older server gets a readiness failure naming the required major rather than a silent substitution. Declaring 19 as a gear-wide baseline was an over-claim: it excluded deployments that have no objection to the gear, only to running a beta PostgreSQL, and it put a pinned beta image on the critical path of every installation. Graph queries execute behind a `GraphQueryPort` in the domain layer with two engine-native backends, both shipped in v1, over a two-query scoped hop that serves whenever neither can (see Consequences):

...
# ADR-0007: Whole-graph analytics ships as its own gear with a read-only connection to the graph schema
...
**ID**: `cpt-cf-graph-storage-adr-analytics-own-gear`

## Context and Problem Statement

ADR-0004 settled *what* analytics is — in-process Rust over a topology-only
...
ADR-0004 acknowledged this only as an obligation on the implementation
...
- ADR-0004 rejected option A (a Python sidecar) partly because "the full graph
...
without reintroducing the whole-graph-over-an-API cost that ADR-0004 rejected,
...
2. **The read-only role can see topology and nothing else.** `SELECT` on `node`
   ...
   ADR-0004's topology-only rule, not a convention the code is trusted to follow.
...
`algorithm_contract_version` and cache identity defined by ADR-0004 are
unchanged and move with the component. This ADR narrows ADR-0004: "analytics in
...
- The DoS coupling ADR-0004 accepted as an implementation obligation stops being
...
- The golden determinism tests of ADR-0004 run unchanged in the new gear,
...
- Good, because the read-only grant enforces ADR-0004's topology-only rule
  mechanically.
...
- Bad, because a million-node topology is serialized, transferred and
  deserialized on every recomputation — the exact objection ADR-0004 used to
...
- ADR-0004 (`cpt-cf-graph-storage-adr-analytics-in-rust`) — what analytics is; narrowed by this ADR on where it runs.
